...
# ============================================================================
# 19. Generate ArgoCD chart values + root-app (iac/argocd/)
# ============================================================================
...
CHART_DIR="$ARGOCD_DIR/chart"
MASTER_FQDN="${CLUSTER_NAME}-master.${DOMAIN}"

mkdir -p "$CLUSTER_OUT/kustomize"

# --- 19a. Generate per-cluster chart values file ---
cat > "$CHART_DIR/values-${CLUSTER_NAME}.yaml" << YAMLEOF
# Auto-generated by generate-cluster.sh — do not edit
# Per-cluster chart values for ${CLUSTER_NAME}
...
gitlabSshUrl: "${GITLAB_SSH_URL}"
portalPrefix: "${PORTAL_PREFIX}"
letsencryptEmail: "${LETSENCRYPT_EMAIL:-letsencrypt@${ROOT_DOMAIN}}"

# Cluster type flags
cni: ${CNI}
helmfileEnv: ${HELMFILE_ENV}
...
# Root-app now points to the Helm chart with per-cluster values
    ...
    - repoURL: ${GIT_REPO_URL}
      ...
      helm:
        valueFiles:
          ...
          - \$values/iac/argocd/chart/values-${CLUSTER_NAME}.yaml
  destination:
